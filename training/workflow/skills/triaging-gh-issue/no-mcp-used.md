...
- The skill calls `gh issue view <N>` via Bash to fetch the issue — it does NOT call any `mcp__` tool.
...
- [ ] `gh issue view <N>` is called to fetch the issue (not any `mcp__` tool)
