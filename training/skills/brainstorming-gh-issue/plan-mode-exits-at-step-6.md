...
- The Phase 3 handoff instructs brainstorming that if it is in plan mode at step 6, it should ask the user to switch to build mode (Tab key) before writing the spec file, since plan mode does not allow file writes.
...
- [ ] The Phase 3 handoff contains an explicit instruction to ask the user to switch to build mode (Tab key) at step 6 if currently in plan mode.
- [ ] The Phase 3 handoff instructs brainstorming to write to `docs/superpowers/specs/`, not to a plan file location.
...
- [ ] Plan mode is not re-entered after step 6.
- [ ] Phase 4 runs successfully after the spec file is written.
