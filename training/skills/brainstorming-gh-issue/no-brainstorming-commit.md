...
- The Phase 4 handoff explicitly instructs brainstorming to write the spec file to disk but NOT commit it.
...
- The spec file is present but untracked when Phase 5 begins.
- Phase 5 locates the file with `ls -t docs/superpowers/specs/*.md | head -1`, posts it, then removes it with plain `rm`.
- No cleanup commit is made in Phase 5.

## Pass Criteria

- [ ] The Phase 4 handoff contains an explicit instruction: do not commit the spec file to git.
...
- [ ] No commit is made in Phase 5 for the spec file removal.
