...
The user runs `/planning-gh-issue 22`. The issue has a valid spec comment. The planning session begins and `writing-plans` produces a draft plan, but the user has not yet approved it.

## Expected Behaviour

- The skill invokes `writing-plans` and waits for user approval.
