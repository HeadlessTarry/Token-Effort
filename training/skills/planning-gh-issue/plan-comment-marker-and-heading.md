...
The user runs `/planning-gh-issue 33`. After the user approves the plan, Phase 4 posts the plan as a GitHub comment.

## Expected Behaviour

- The comment body starts with `<!-- planning-gh-issue:plan -->` on its own line.
- The comment contains the heading `## 🤖📋 Implementation Plan`.
- The footer includes "Please review carefully before approving."
- The footer instructs the user to remove the `pending-review` label and advance the issue to the next project board status.

## Pass Criteria

- [ ] The comment body begins with `<!-- planning-gh-issue:plan -->` as the very first line.
