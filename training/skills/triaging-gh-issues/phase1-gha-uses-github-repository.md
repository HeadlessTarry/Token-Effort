...
The skill is invoked in a GitHub Actions environment. `GITHUB_ACTIONS` is set to `true` and `GITHUB_REPOSITORY` is set to `TheTarry/Token-Effort`. One unlabelled open issue exists: a feature request to add dark mode support.
