...
- `issue_write` is called for both issues to apply the classified labels.
- `add_issue_comment` is not called for either issue.

## Pass Criteria

- [ ] No confirmation table is displayed and no user prompt is issued.
- [ ] `issue_write` is called for the CSV export issue with label `enhancement`.
- [ ] `issue_write` is called for the documentation issue with label `documentation`.
- [ ] `add_issue_comment` is never called.
