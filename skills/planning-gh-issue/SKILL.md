...
Fetches a GitHub issue and its approved design spec, moves the issue to **Planning** status, then invokes the `writing-plans` skill to run an interactive planning session. After the user approves the plan, posts it as a comment on the issue and applies the `pending-review` label.
...
**Use when:**
- A GitHub issue has an approved design spec comment (produced by `brainstorming-gh-issue`) and is ready to be planned
...
- You want to plan something not tied to a GitHub issue — use the `writing-plans` skill directly instead
...
> "No issue number found in args or branch name. Run as `/planning-gh-issue <N>`."
...
> "No design spec found on issue #N. Please run `/brainstorming-gh-issue <N>` first and get the spec approved before planning."
...
**Detect re-entry:** Search all entries in the `comments` array for one whose `body` starts with `<!-- planning-gh-issue:plan -->`:

- **No plan comment → fresh planning run.** Proceed to Phase 3 with issue context and spec content. There is no prior plan.
- **Plan comment found → re-entry mode.** Strip the `<!-- planning-gh-issue:plan -->` marker line and extract the prior plan body. Load both the issue context, the spec content, and the prior plan into Phase 3 as a continuation.

### Phase 3 — Move to Planning and invoke `writing-plans`

**Move issue to Planning status:**

Invoke the `move-issue-status` skill with the issue number and status "Planning".
...
- After the user approves the plan, do **not** call `subagent-driven-development`, `executing-plans` or any other build/execution skill. Proceed to Phase 4 of this skill instead.
...
`writing-plans` will have written the approved plan to `docs/superpowers/plans/`. Find the most recently created file:
...
```bash
gh label create "pending-review" --color "#FEF2C0" --description "Plan posted, awaiting human approval"
```
...
- **Blocking on the Planning status move failure** — `move-issue-status` errors are non-fatal. Log the warning and continue. Never stop planning because of a status update failure.
- **Posting the plan before the user approves it** — Phase 4 must not run until the user has explicitly approved the plan within the `writing-plans` session. Do not call `gh issue comment` or `gh issue edit` during Phase 3.
- **Not reading the plan file before posting** — always locate and read the file that writing-plans wrote with `ls -t docs/superpowers/plans/*.md | head -1`. Do not reconstruct the plan content from memory.
- **Forgetting the HTML comment marker** — the plan comment must begin with `<!-- planning-gh-issue:plan -->` on its own line so future re-entry runs can locate it reliably.
- **Invoking execution skills after plan approval** — the Phase 3 handoff instructs writing-plans to stop after the user approves the plan. Do not invoke `subagent-driven-development`, `executing-plans` or any build skill; proceed to Phase 4 instead.
...
- **Stripping the wrong marker line** — the spec comment starts with `<!-- brainstorming-gh-issue:spec -->` and the plan comment starts with `<!-- planning-gh-issue:plan -->`. Ensure you strip the correct marker for each.
...
- [ ] When no args given and branch has no digits: stopped with a message containing the suggested invocation `/planning-gh-issue <N>`
...
- [ ] Identified the absence of `<!-- planning-gh-issue:plan -->` comment and proceeded as a fresh planning run
- [ ] In re-entry mode: loaded prior plan content (marker stripped) into Phase 3 alongside issue context and spec
- [ ] Invoked `move-issue-status <N> "Planning"` before invoking writing-plans
...
- [ ] Phase 4 located the plan file with `ls -t docs/superpowers/plans/*.md | head -1`
- [ ] Phase 4 read the plan file content before constructing the GitHub comment
- [ ] The plan comment body starts with `<!-- planning-gh-issue:plan -->`
