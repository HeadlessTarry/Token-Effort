...
**When no existing file is present:** ensure the `.github/` directory exists, then write the full file from scratch with one entry per detected ecosystem. Always use `directory: /`.
...
- **Not creating `.github/` directory before writing** — when no existing file is present, ensure the `.github/` directory exists before writing `dependabot.yml`.
...
- [ ] Ensured `.github/` directory exists before writing `dependabot.yml` when no prior file existed
