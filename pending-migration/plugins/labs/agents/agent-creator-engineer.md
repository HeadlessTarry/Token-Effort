...
If the user's request names or provides a path to an existing agent file (e.g. `plugins/<plugin>/agents/<name>.md`) → Review mode. Otherwise → Create mode.
...
Create `plugins/labs/agents/<name>.md` using the Agent File Template below. Follow the best practices from the fetched docs. Apply the Repo Checklist below. (`Write` is used here for the initial file creation only — all subsequent edits, including in Review mode, use `Edit`.)
...
- **File location:** `plugins/labs/agents/<name>.md` (flat file, not a subdirectory)
...
1. File is at `plugins/labs/agents/<name>.md` (flat file, not a subdirectory)
