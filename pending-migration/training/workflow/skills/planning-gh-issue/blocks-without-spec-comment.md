...
The user runs `/token-effort-workflow:planning-gh-issue 42`. The issue exists and has 3 comments, but none starts with `<!-- brainstorming-gh-issue:spec -->`. The brainstorming phase has not been run for this issue.
...
- Execution stops immediately with a clear error message telling the user to run `/token-effort-workflow:brainstorming-gh-issue 42` first.
