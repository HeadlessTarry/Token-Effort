...
- Phase 3 invokes `token-effort-workflow:move-issue-status 88 "Planning"` before invoking `superpowers:writing-plans`.
- The status move completes successfully.
- `superpowers:writing-plans` is then invoked.

## Pass Criteria

- [ ] `token-effort-workflow:move-issue-status 88 "Planning"` is called in Phase 3.
