...
The install scripts (`install.sh`, `install.ps1`) only copied Token-Effort's own skills and agents to the OpenCode config directory. New users cloning Token-Effort could not get a working setup from a single `install` command — they had to manually install third-party dependencies (superpowers, addyosmani skills, ponytail), configure `opencode.json` plugin entries, and manage each dependency separately.
...
**Scope:** Only `superpowers` is included in the initial `vendor.json`. Adding `ponytail` or `addyosmani-agent-skills` is deferred to issue #186.
...
- **Deferred:** Config file copying (e.g., Zed rules for ponytail) — deferred until a vendor actually requires it (YAGNI).
